...
#[test]
pub fn given_a_driver_project_when_verify_signature_is_false_then_it_builds_successfully() {
    // Input CLI args
    let cwd = PathBuf::from("C:\\tmp");
    let profile = Profile::Dev;
    let target_arch = TargetArch::X64;
...
#[test]
pub fn given_a_driver_project_when_profile_is_dev_and_target_arch_is_host_then_it_builds_successfully(
) {
    // Input CLI args
    let cwd = PathBuf::from("C:\\tmp");
    let profile = Profile::Dev;
...
impl TestSetupPackageExpectations for TestPackageAction {
    ...
    fn expect_stampinf(
        ...
        .expect("Wdk metadatat must be available");
